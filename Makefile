...
FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_ID ?=
FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_SECRET ?=
FRUGAN_AYUF_CACHE_BUSTING_ENABLED ?= false
...
#FIXED: safe.directory avoids Github fatal error: detected dubious ownership in repository
	...
	@$(DOCKER_COMPOSE) exec -u$(WORDPRESS_CONTAINER_USER) $(WORDPRESS_CONTAINER_NAME) sh -c "sed -i '/define('\''FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_ID'\'',/d' $${WORDPRESS_BASE_DIR:-/bitnami/wordpress}/wp-config.php && sed -i '1a define('\''FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_ID'\'', '\''${FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_ID}'\'');' $${WORDPRESS_BASE_DIR:-/bitnami/wordpress}/wp-config.php"
	@$(DOCKER_COMPOSE) exec -u$(WORDPRESS_CONTAINER_USER) $(WORDPRESS_CONTAINER_NAME) sh -c "sed -i '/define('\''FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_SECRET'\'',/d' $${WORDPRESS_BASE_DIR:-/bitnami/wordpress}/wp-config.php && sed -i '2a define('\''FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_SECRET'\'', '\''${FRUGAN_AYUF_GOOGLE_OAUTH_CLIENT_SECRET}'\'');' $${WORDPRESS_BASE_DIR:-/bitnami/wordpress}/wp-config.php"
	@$(DOCKER_COMPOSE) exec -u$(WORDPRESS_CONTAINER_USER) $(WORDPRESS_CONTAINER_NAME) sh -c "sed -i '/define('\''FRUGAN_AYUF_CACHE_BUSTING_ENABLED'\'',/d' $${WORDPRESS_BASE_DIR:-/bitnami/wordpress}/wp-config.php && sed -i '3a define('\''FRUGAN_AYUF_CACHE_BUSTING_ENABLED'\'', ${FRUGAN_AYUF_CACHE_BUSTING_ENABLED});' $${WORDPRESS_BASE_DIR:-/bitnami/wordpress}/wp-config.php"
